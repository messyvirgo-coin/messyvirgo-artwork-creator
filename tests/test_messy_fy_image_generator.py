...
class MessyFyImageGeneratorTests(unittest.TestCase):
    def test_messy_fy_prompt_library_composes_brand_prompt_with_optional_hint(self):
        ...
        self.assertIn("Messy Virgo brand", prompt)
        self.assertIn("complete brand redesign", prompt)
        self.assertIn("never garble, misspell, translate, or invent copy", prompt)
        self.assertIn("Replace blueprint grids", prompt)
        self.assertIn("replace every source font", prompt)
        ...
        self.assertIn("monospaced or typewriter text", library.negative_prompt)
